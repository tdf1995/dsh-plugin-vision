...
/**
 * Windows curl.exe cannot open paths containing non-ASCII characters
 * (argv is passed in the ANSI codepage), and curl `--config` files treat
 * backslashes as escape characters. All curl-facing paths therefore use
 * FORWARD slashes, and live on an ASCII path: the workspace when its root is
 * pure ASCII, otherwise a unique per-call directory under the OS temp dir
...
function tempDir(workspaceRoot) {
  if (isAsciiPath(workspaceRoot)) {
    return { dir: workspaceRoot.replace(/\\/g, '/') + '/.dsh-vision', isTmp: false };
  }
  return {
    dir: tmpdir().replace(/\\/g, '/') + '/dsh-vision-' + Date.now().toString(36) + '-' + Math.floor(Math.random() * 1e6).toString(36),
    isTmp: true,
  };
}
...
/** Resolve the sandbox policy for the given session id (browser-paste route). */
function resolvePolicyForSessionId(ctx, sessionId) {
  const policy = ctx.get('sandboxPolicy');
  if (policy === undefined) return undefined;
  let session;
  ...
  if (session === undefined) return resolvePolicy(ctx);
  try {
    return policy.resolve({ session });
  } catch (e) {
    return resolvePolicy(ctx);
  }
}

/** Read a bounded JSON request body (node:http IncomingMessage). */
...
/**
 * Handle one pasted image from the browser: decode base64 and write it to the
 * session workspace uploads dir. Returns { path, mime }.
 */
async function savePastedImage(ctx, args) {
  const fs = ctx.fs;
  const shell = ctx.shell;
  const policy = resolvePolicyForSessionId(ctx, args && args.sessionId);
  ...
  const command = IS_WIN
    ? 'New-Item -ItemType Directory -Force -Path $env:VISION_UP | Out-Null; [System.IO.File]::WriteAllBytes($env:VISION_OUT, [System.Convert]::FromBase64String([System.IO.File]::ReadAllText($env:VISION_B64).Trim())); Remove-Item $env:VISION_B64 -Force'
    : 'mkdir -p "$VISION_UP" && base64 --decode "$VISION_B64" > "$VISION_OUT" && rm -f "$VISION_B64"';
  let result;
  try {
...
async function maybeDownscale(ctx, target, ext, workspaceRoot, cfg, signal) {
  ...
  try {
    ...
    const { dir } = tempDir(workspaceRoot);
...
export default {
  ...
  apply(ctx, config) {
    ...
    /** Fire-and-forget deletion of request temp files (key-bearing cfg, payload, resp) and the per-call temp dir. */
    function cleanupFiles(paths, dirPath) {
      ...
      if (dirPath) {
        try {
          const argv = IS_WIN ? ['cmd.exe', '/d', '/c', 'rd', '/s', '/q', dirPath] : ['rm', '-rf', dirPath];
          const h = subprocess.spawn({
            argv,
            cwd: process.cwd(),
            stdio: { stdin: 'ignore', stdout: { maxBytes: 1024 }, stderr: { maxBytes: 1024 } },
    ...
    async function runVision(args, exec) {
      ...
      const { dir, isTmp } = tempDir(workspaceRoot);
      ...
      for (let ci = 0; ci < candidates.length; ci += 1) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
          ...
          try {
            ...
            cleanupFiles([cfgPath, payloadPath, respPath], isTmp ? dir : undefined);
            return out;
          } catch (e) {
            attemptError = e;
    ...
    // Browser paste endpoint: the client half POSTs pasted/dropped images here.
    // The webServer service may not be mounted yet when this row activates
    // (boot ordering), so registration retries until it appears; the tools keep
    // working without it (non-web profiles just skip the paste route).
    function registerPasteRoute() {
      const ws = ctx.get('webServer');
      if (ws === undefined) return false;
      ctx.effect(
        () => ws.register({
          kind: 'exact',
          path: '/vision/save-image',
          async handler(req, res) {
            ...
            try {
              const raw = await readRequestBody(req, 32 * 1024 * 1024);
              const args = JSON.parse(raw);
              const out = await savePastedImage(ctx, args);
              res.statusCode = 200;
        ...
        'dsh-plugin-vision: /vision/save-image',
      );
      return true;
    }
    if (!registerPasteRoute()) {
      let disposed = false;
      ctx.effect(() => () => { disposed = true; });
      let tries = 0;
      const tick = () => {
        if (disposed) return;
        if (registerPasteRoute()) return;
        tries += 1;
...
export { isAsciiPath, tempDir, savePastedImage };
