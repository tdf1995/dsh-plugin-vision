...
/** Run curl.exe directly through the subprocess seam (no shell startup). */
async function runCurl(subprocess, cfgPath, workspaceRoot, signal) {
  const handle = subprocess.spawn({
    argv: ['curl.exe', '--config', cfgPath],
...
async function maybeDownscale(ctx, target, ext, workspaceRoot, cfg, signal) {
  ...
  try {
    ...
    const outPath = dir + '/resized-' + Date.now() + '.jpg';
    ...
    const handle = subprocess.spawn({
      argv: ['pwsh.exe', '-NoProfile', '-NonInteractive', '-Command', script],
      cwd: workspaceRoot || process.cwd(),
...
export default {
  inject: ['fs', 'shell', 'timer', 'subprocess'],
  config: Config,
  apply(ctx) {
    const fs = ctx.fs;
    ...
    async function runVision(args, exec) {
      ...
      if (provider !== 'auto') {
        ...
        const key = await resolveKey(ctx, p.keyRef);
      ...
      for (let ci = 0; ci < candidates.length; ci += 1) {
        ...
        for (let attempt = 0; attempt < maxAttempts; attempt += 1) {
          ...
          try {
            ...
            attemptError = e;
            if (isRateLimited(e.message) && attempt < maxAttempts - 1) {
        ...
        if (ci < candidates.length - 1 && (isRateLimited(lastError.message) || isNetworkError(lastError.message))) {
    ...
    const setKeyTool = defineTool({
      ...
      async execute(args) {
        ...
        if (credentials === undefined) {
          return {
            content:
              '凭据服务不可用（credentials 未挂载）。请把 ' + p.keyRef + ' 写入环境变量（或 ~/.dsh/.credentials.yaml）后重启 DSH。',
          };
        }
        await credentials.set(p.keyRef, value);
        return { content: '已保存 ' + p.label + ' API Key（引用 ' + p.keyRef + '），下一次 see_image 调用立即生效。' };
      },
    });
    ...
    const statusTool = defineTool({
      ...
      async execute() {
        ...
        for (const name of ['gemini', 'glm']) {
          const p = PROVIDERS[name];
          const env = process && process.env ? process.env[p.keyRef] : undefined;
          const credentials = ctx.get('credentials');
          if (credentials !== undefined) {
            try {
              const info = await credentials.describe(p.keyRef);
              if (info && info.configured) {
                lines.push(p.label + ' (' + p.keyRef + '): 已配置，来源 ' + (info.source || '?') + (info.writable ? '，可写入' : ''));
                continue;
              }
            } catch (e) { /* fall through */ }
          }
          lines.push(p.label + ' (' + p.keyRef + '): ' + (env ? '已配置（环境变量）' : '未配置'));
        }
