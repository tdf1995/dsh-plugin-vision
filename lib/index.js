 ...
 * (argv is passed in the ANSI codepage). Keep curl-facing temp files on an
 * ASCII path: the workspace when its root is pure ASCII, otherwise a unique
 * per-call directory under the OS temp dir (cleaned up after the call).
 */
...
function tempDir(workspaceRoot) {
  if (isAsciiPath(workspaceRoot)) return { dir: workspaceRoot + '/.dsh-vision', isTmp: false };
  return {
    dir: tmpdir() + '/dsh-vision-' + Date.now().toString(36) + '-' + Math.floor(Math.random() * 1e6).toString(36),
    isTmp: true,
  };
}
